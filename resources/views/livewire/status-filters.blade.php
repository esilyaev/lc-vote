<nav class="flex items-center justify-between text-xs text-gray-400">
    ...
    <div>
        <ul class="flex uppercase font-semibold space-x-10 ">
            <li><a wire:click.prevent="SetStatus('Answered')" href="#" class=" transition duration-2 ease-in border-b-4 pb-3 hover:border-cyan-500 hover:text-gray-900 @if($status === 'Answered') border-cyan-500 text-gray-900 @endif">Answered ({{ $statusCount['answered'] }})</a></li>
            <li><a wire:click.prevent="SetStatus('Closed')" href="#" class=" transition duration-2 ease-in border-b-4 pb-3 hover:border-gray-700 hover:text-gray-900 @if($status === 'Closed') border-gray-700 text-gray-900 @endif">Closed ({{ $statusCount['closed'] }})</a></li>
        </ul>
    </div>
</nav>